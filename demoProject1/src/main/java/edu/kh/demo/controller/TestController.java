...
// IOC (제어의 반전) : 객체의 새성 및 생명주의기 권한이 개발자가 아닌, 
// 						프레임워크에게 있다.

@Controller // 요청/응답을 제어하는 역할인 컨트롤러임을 명시 + Bean 등록
// @RequestMapping("/test") // 요청주소를 mapping해주는것
public class TestController {
	...
	*/
	...
	//@RequestMapping(value="/test", method=RequestMethod.GET) // GET 방식만 매핑하겠다
	@RequestMapping("/test") // /test 요청 시 testMethod가 매핑하여 처리함
	public String testMethod() {
		System.out.println("/test 요청 받음");
		
		/*
		 * Controller 메서드의 반환형이 String 인(public String) 이유!
		 * -> 메서드에서 반환되는 문자열이
		...
		return "test"; // 접두사 + 반환값 + 접미사 경로로 html로 forward
	}
