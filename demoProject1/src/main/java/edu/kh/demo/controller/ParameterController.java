...
public class ParameterController {
	...
	public String paramTest1(HttpServletRequest req) {
		...
		log.debug("inputAge : " + inputAge); // 콘솔창에서 확인 가능
	 ...
	 * -> required=true인때 파라미터가 존재하지 않는다면(name속성이 없는 경우) 400(Bad Request) 에러 발생
	 * 		(required=false인데 name속성이 없는 경우 에러 발생하지 않음 but 콘솔에는 찍힌다)
	 * -> "" (빈문자열)일 때는 에러 발생 X 
	...
	public String paramTest3(@RequestParam("color") String[] colorArr, 
							...
							// (Map은 제출된 파라미터를 전부다 묶어서 갖고오기 때문에 name속성값이 필요 없다.)
			) {
	...
	// index.html 등으로 부터 전달받은 파라미터의 name속성값이
	...
	public String paramTest4(/*@ModelAttribute*/ Member inputMember) {
		/* @ModelAttribute 내부적으로 아래와 같이 수행된다
		...
		*/
		...
		return "redirect:main"; // 상대경로 작성법(앞에 '/' 없이 작성)
		// /param/main
		// 상대경로에서 중요한 부분 ! -> 현재위치!!!
		// -> 현재 경로의 가장 마지막 레벨의 주소값(http://localhost/param/main에서 main)을
