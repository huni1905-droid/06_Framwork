...
public class ParameterController {
	...
	@PostMapping("test3")  //  /param/test3 POST 방식 요청 매핑
	public String paramTest3(@RequestParam("color") String[] colorArr, /* 배열로 얻어오기*/
							@RequestParam("fruit") List<String> fruitList, /* List로 얻어오기*/
	...
	// *** 주의 사항 ***
	// - DTO에 기본생성자가 필수로 존재해야 함
	// - DTO에 setter가 필수로 존재해야 함
	
	// @ModelAttribute를 이용해 값이 필드에 세팅된 
	// 객체(Member inputMember)를 "커맨드 객체" 라고 부름
	
	// @Member inputMember 생략 가능!
	@PostMapping("test4")  //  /param/test3 POST 방식 요청 매핑
	public String paramTest4(/*@ModelAttribute*/ Member inputMember) {
	
		log.debug("inputMember: " + inputMember);
		// return"redirect:/param/main"; // 절대경로 작성법 
		return"redirect:main"; // 상대경로 작성법
		// 상대경로에서 중요한 부분! -> 현재위치!!!
		// -> 현재 경로의 가장 마지막 레벨의 주소값을 
		// "redirect : 이 주소"로 변경하기 때문
		// ex) http://localhost/param/main 에서 가장 마지막 부분인 main을 
		// 상대경로에서 main으로 갈아끼우겠다
		// (상대경로 작성시 앞에 "/"를 붙이지 않는다! 주의!!!)
	}
