...
public class LoggingAspect {
	...
	public void beforeController(JoinPoint jp){
		...
		if(req.getSession().getAttribute("loginMember") != null) {
			...
			String memberEmail = ((Member)req.getSession().getAttribute("loginMember")).getMemberEmail();
								// req.getSession().getAttribute("loginMember") : Object 타입(뭐든 다 받아줘야되서)
	...
	// 1) @Around 사용 시 반환형 Object 여야 한다
	 ...
	 * @return
	 */
	@Around("execution(* edu.kh.project..*ServiceImpl*.*(..))")
	public Object aroundServiceImpl(ProceedingJoinPoint pjp) throws Throwable{
		// Throwable - 예외 처리의 최상위 클래스(그 밑으로 Exception과 Error가 있다)
