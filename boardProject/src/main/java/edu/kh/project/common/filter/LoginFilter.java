...
/*
 * Filter : 요청, 음답 시 걸러내거나 추가할 수 있는 객체
 * 
 * [필터 클래스 생성 방법]
...
public class LoginFilter implements Filter{
	...
	public void doFilter(ServletRequest request, 
		...
		// session 이 필요함 -> 왜? -> loginMember가 session에 담기기 때문
		...
		// 현재 요청의 URI 를 가져옴
		String path = req.getRequestURI(); //   /myPage/profile 형태로 들어옴
		
		// 요청 URI 가 "/myPage/profile" 로 시작하는지 확인
		if(path.startsWith("/myPage/profile/")) {
			// 필터를 그냥 통과하도록 함
			// (/myPage/profile/ == 마지막 '/'있어서 profile 이하를 의미)
		...
		if( session.getAttribute("loginMember") == null) {
			...
			// 다음 필터로, 또는 다음필터가 없다면 DispatcherServlet으로 요청,응답 전달
			chain.doFilter(request, response);
