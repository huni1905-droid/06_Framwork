...
/*
 * AOP(Aspect-Oriented Programming) 분산되어있는 관심사/관점을 모듈화 시키는 기법
 ...
 * - @Before(포인트컷) : 대상 메서드(포인트컷) 실행 전에 Adivice를 실행함.
 * - @After(포인터컷)  : 대상 메서드 실행 후에 Advice를 실행함.
 * - @Around(포인트컷) : 대상 메서드 실행 전/후로 Advice를 실햄함(@Before + @After)
 * 
 * */

@Component // bean 등록(클래스 상단에 작성 cf. @Bean은 메서드 상단에 작성)
