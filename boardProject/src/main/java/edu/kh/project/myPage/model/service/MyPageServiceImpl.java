...
public class MyPageServiceImpl implements MyPageService {
	...
	public int fileUpload2(MultipartFile uploadFile, int memberNo) throws Exception {
		...
		// 마지막에 '/' 없다면 test 폴더 안이 아닌 uploadFiles 폴더 안에 저장된다
		
		// 클라이언트가 파일이 저장된 폴더에 접근할 수 있는 주소(요청 주수)
		String webPath = "/myPage/file/"; // DB에 이 경로로 저장
	...
	public int profile(MultipartFile profileImg, Member loginMember) throws Exception {
		...
		if(result > 0) { // DB에 업데이트 성공
			...
			// 프로필 이미지를 없앤 경우(NULL로 수정한 경우)를 제외한 경우
			// -> 업로드한 이미지가 있을 경우
			if( !profileImg.isEmpty() ){
				// 파일을 서버 지정된 폴더에 저장
